...
export default function DarkVeil({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      // resolutionScale 用作 DPR 倍率，移动端可降至 < 1 以减少 GPU 负担。
      // 注意：不能乘到 setSize(w,h) 上，那会改变 canvas 的 CSS 尺寸，
      //      导致 fragCoord / uResolution 比例错位，图案会偏到一角。
    ...
    const resize = () => {
      ...
      // 用容器自身的尺寸渲染；DPR 已经按 resolutionScale 缩放过，
      // 这样 canvas CSS 尺寸 = 容器尺寸，shader 中 fragCoord 与 uResolution 比例正确。
    ...
    // 2) iOS Safari 在地址栏伸缩时不触发 resize，但视口/父元素尺寸会变化，
    //    用 ResizeObserver 监听 parent 自身尺寸，能稳定捕捉到这些场景。
    let ro = null;
    if (typeof ResizeObserver !== 'undefined') {
      ro = new ResizeObserver(resize);
      ro.observe(parent);
    }
    // 3) 移动端方向改变 / 视觉视口缩放时也强制刷新一次
